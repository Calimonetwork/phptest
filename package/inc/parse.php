...
#													$table->attributes['class'].=' float';
	...
	private function formatParser($line) {
		...
		for($j=0;$j<$line_len;self::nextChar($line,$j)) {
			if(self::startsWith($line, 'http', $j) && preg_match('/(https?:\/\/[^ ]+\.(jpg|jpeg|png|gif))(?:\?([^ ]+))?/i', $line, $match, 0, $j)) {
				...
				if($this->imageAsLink)
					$innerstr = '<span class="alternative">[<a class="external" target="_blank" href="/files/'.($match[1]?($match[1]=='' || substr($match[1], 0, -1)==''?'':substr($match[1], 0, -1).'__'):rawurlencode($this->WikiPage->title).'__').$match[2].'">image</a>]</span>';
				else {
					$paramtxt = '';
					$csstxt = '';
					if(!empty($match[3])) {
	...
	private function linkProcessor($text, $type) {
		...
		elseif(preg_match('/^파일:(.+)$/', $href[0], $category)) {
			...
			$paramtxt .= ($csstxt!=''?' style="'.$csstxt.'"':'');
			return '<a href="'.$this->prefix.'/'.self::encodeURI($category[0]).'" title="'.htmlspecialchars($category[0]).'"><img src="/assets/image/upload/'.self::encodeURI($category[0]).'"'.$paramtxt.'></a>';
		}
		else {
...
function parse($input_text){
	$wPage = new PlainWikiPage($input_text);
